...
///
/// # Arguments
/// * `delimiter` - The delimiter character (single UTF-8 character)
/// * `status` - Output parameter for error status (0 = success, negative = error)
///
/// # Returns
/// * Pointer to the newly created CharDelimiterSplit pre-tokenizer, or null on error
///
/// # Safety
/// * The returned pointer must be freed using `tokenizers_char_delimiter_split_free`
#[no_mangle]
pub extern "C" fn tokenizers_char_delimiter_split_new(
    delimiter: *const c_char,
    status: *mut i32,
) -> *mut CharDelimiterSplit {
    if status.is_null() {
        return ptr::null_mut();
    }
    ...
    let chars: Vec<char> = delimiter_str.chars().collect();
    if chars.len() != 1 {
        set_status(status, -3);
        return ptr::null_mut();
    }

    let pre_tokenizer = CharDelimiterSplit::new(chars[0]);
    set_status(status, 0);
    Box::into_raw(Box::new(pre_tokenizer))
}

/// Pre-tokenizes a string using CharDelimiterSplit.
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the CharDelimiterSplit pre-tokenizer
...
///
/// # Safety
/// * `pre_tokenizer` must be a valid pointer from `tokenizers_char_delimiter_split_new`
/// * `input` must be a valid UTF-8 null-terminated string
/// * `output` must be null or point to a buffer of at least `output_len` bytes
#[no_mangle]
pub extern "C" fn tokenizers_char_delimiter_split_pre_tokenize_str(
    pre_tokenizer: *mut CharDelimiterSplit,
    input: *const c_char,
    output: *mut u8,
    output_len: usize,
    status: *mut i32,
...
/// Frees a CharDelimiterSplit pre-tokenizer.
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the pre-tokenizer to free (can be null)
///
/// # Safety
/// * `pre_tokenizer` must be null or a valid pointer from `tokenizers_char_delimiter_split_new`
/// * After calling this function, the pointer must not be used again
#[no_mangle]
pub extern "C" fn tokenizers_char_delimiter_split_free(pre_tokenizer: *mut CharDelimiterSplit) {
    if !pre_tokenizer.is_null() {
        unsafe {
...
mod tests {
    ...
    #[test]
    fn test_char_delimiter_split_new() {
        let delimiter = CString::new(",").unwrap();
        let mut status = 0;
        let pre_tokenizer = tokenizers_char_delimiter_split_new(delimiter.as_ptr(), &mut status);
        assert_eq!(status, 0);
        assert!(!pre_tokenizer.is_null());
        tokenizers_char_delimiter_split_free(pre_tokenizer);
    }

    #[test]
    fn test_char_delimiter_split_pre_tokenize_comma() {
        let delimiter = CString::new(",").unwrap();
        let mut status = 0;
        let pre_tokenizer = tokenizers_char_delimiter_split_new(delimiter.as_ptr(), &mut status);
        assert_eq!(status, 0);

        let input = CString::new("hello,world,test").unwrap();
        let mut buffer = vec![0u8; 1024];
        let written = tokenizers_char_delimiter_split_pre_tokenize_str(
            pre_tokenizer,
            input.as_ptr(),
            buffer.as_mut_ptr(),
            buffer.len(),
    ...
    #[test]
    fn test_char_delimiter_split_invalid_multi_char() {
        let delimiter = CString::new("ab").unwrap();
        let mut status = 0;
        let pre_tokenizer = tokenizers_char_delimiter_split_new(delimiter.as_ptr(), &mut status);
        assert_eq!(status, -3);
        assert!(pre_tokenizer.is_null());
