...
/// Creates a new ByteLevel pre-tokenizer instance.
///
/// This pre-tokenizer takes care of replacing all bytes of the given string
/// with a corresponding representation, as well as splitting into words.
///
/// # Arguments
/// * `add_prefix_space` - Whether to add a space to the first word if there isn't already one
/// * `use_regex` - Whether to use the GPT2 specific regex for splitting on whitespace
/// * `status` - Pointer to status code (0 = success, -1 = null pointer, other = error)
///
/// # Returns
/// Pointer to the created ByteLevel pre-tokenizer, or null pointer on failure.
///
/// # Safety
/// The returned pointer must be freed with `tokenizers_byte_level_pre_tokenizer_free`.
#[no_mangle]
pub extern "C" fn tokenizers_byte_level_pre_tokenizer_new(
    ...
    let mut pre_tokenizer = ByteLevel::default();
    pre_tokenizer.add_prefix_space = add_prefix_space;
    pre_tokenizer.use_regex = use_regex;
...
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the ByteLevel pre-tokenizer
...
#[no_mangle]
pub extern "C" fn tokenizers_byte_level_pre_tokenizer_pre_tokenize_str(
    pre_tokenizer: *const ByteLevel,
    input: *const c_char,
    output: *mut c_char,
    output_len: usize,
    status: *mut i32,
...
/// Returns the alphabet used by the ByteLevel pre-tokenizer.
///
/// Since the ByteLevel works at the byte level, it encodes each byte value to a unique
/// visible character. This means there is a total of 256 different characters.
///
/// Uses a two-call pattern:
/// 1. Call with output=null to get required buffer size
/// 2. Call with allocated buffer to get the JSON string
///
/// # Arguments
/// * `output` - Output buffer for JSON array of characters, or null to get required size
/// * `output_len` - Length of output buffer in bytes
/// * `status` - Pointer to status code (0 = success, -1 = null pointer, other = error)
///
/// # Returns
...
/// or number of bytes written (excluding null terminator) if output is provided.
#[no_mangle]
pub extern "C" fn tokenizers_byte_level_pre_tokenizer_alphabet(
    output: *mut c_char,
    output_len: usize,
    status: *mut i32,
) -> usize {
    ...
    let alphabet = ByteLevel::alphabet();
...
/// Frees a ByteLevel pre-tokenizer instance.
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the ByteLevel pre-tokenizer to free
///
/// # Safety
/// The pointer must have been created with `tokenizers_byte_level_pre_tokenizer_new` and must not be used after calling this function.
#[no_mangle]
pub extern "C" fn tokenizers_byte_level_pre_tokenizer_free(pre_tokenizer: *mut ByteLevel) {
