...
/// Creates a new BertPreTokenizer instance.
///
...
///
/// # Returns
/// Pointer to the created BertPreTokenizer, or null pointer on failure.
///
/// # Safety
/// The returned pointer must be freed with `tokenizers_bert_pre_tokenizer_free`.
#[no_mangle]
pub extern "C" fn tokenizers_bert_pre_tokenizer_new(status: *mut i32) -> *mut BertPreTokenizer {
    if status.is_null() {
        return ptr::null_mut();
    }
    set_status(status, -1);
...
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the BertPreTokenizer
...
#[no_mangle]
pub extern "C" fn tokenizers_bert_pre_tokenizer_pre_tokenize_str(
    pre_tokenizer: *const BertPreTokenizer,
    input: *const c_char,
    output: *mut c_char,
    output_len: usize,
    status: *mut i32,
...
/// Frees a BertPreTokenizer instance.
///
/// # Arguments
/// * `pre_tokenizer` - Pointer to the BertPreTokenizer to free
///
/// # Safety
/// The pointer must have been created with `tokenizers_bert_pre_tokenizer_new` and must not be used after calling this function.
#[no_mangle]
pub extern "C" fn tokenizers_bert_pre_tokenizer_free(pre_tokenizer: *mut BertPreTokenizer) {
